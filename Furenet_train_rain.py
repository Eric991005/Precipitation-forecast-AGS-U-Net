...
from FURENet.Attention_FURENet_forecasting_rain import FURENet
import random
...
class FURENetTesting(TestingTemplate):
    ...
    def forward(self, inputs, out_len: int = 10):
        # 重写此方法以处理三个输入张量
        z, zdr = inputs
        return self.model(z, zdr, out_len=out_len)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.dbz[idx], self.zdr[idx], self.y[idx]
