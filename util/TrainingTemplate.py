...
class TrainingTemplate(object):
    def __init__(self, model: nn.Module, train_loader: DataLoader, test_loader: DataLoader,
                 ...
                 start_save: int = 5, visualize: bool = False):
        if device is None:
    ...
    def save(self, epoch, loss):
        try:
            ...
            checkpoint_path = self.to_save.joinpath("checkpoint.pth")
            torch.save(obj=states, f=checkpoint_path)
            print(f'Saved checkpoint to {checkpoint_path}')

            if self.is_best(loss=loss):
                best_path = self.to_save.joinpath("best.pth")
